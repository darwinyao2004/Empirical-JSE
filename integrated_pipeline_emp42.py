...
- Use the remaining days (~21 trading days) to compute each stock's monthly return (compounded),
  and evaluate OOS portfolio return within the same file
...
- results_emp42/{PCA,JSE}/PortfolioA_GMV_returns.csv
- results_emp42/summary.csv
...
DEFAULT_OUT_COV_ROOT = "covariance_outputs_emp42"
DEFAULT_OUT_PORT_ROOT = "portfolio_outputs_emp42"
DEFAULT_OUT_RESULTS_ROOT = "results_emp42"
...
def integrated_run(
    ...
    for fpath in files:
        ...
        # --- OOS evaluation within the same file (third month) ---
        asset_oos = compound_return(X_oos_raw)  # (p,)
        # No renormalization: keep weights exactly as produced by solver
        r_pca = float(np.dot(w_pca, asset_oos))
        r_jse = float(np.dot(w_jse, asset_oos))

        dt = month_end_timestamp(yyyymm)
        backtests["PCA"].append(BacktestPoint(date=dt, realized_return=r_pca))
        backtests["JSE"].append(BacktestPoint(date=dt, realized_return=r_jse))
    ...
    for method in METHODS:
        ...
        if not pts:
            continue
        pts_df = pd.DataFrame(
            {"date": [p.date for p in pts], "realized_return": [p.realized_return for p in pts]}
        ...
        ensure_dir(out_dir)
        out_curve = out_dir / f"PortfolioA_GMV_returns.csv"
        pts_df.to_csv(out_curve, index=False)

        summary_rows.append(
            dict(method=method, portfolio_type="PortfolioA_GMV", n_points=len(pts_df), mean_return=mu, var_return=var)
        )
